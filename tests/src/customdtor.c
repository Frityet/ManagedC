...
static void MyStruct_free(struct MyStruct *mystruct)
{
    (^bool(void) {
        ...
        TEST_EXPR(strncmp(mystruct->reference, "Hello", lengthof(mystruct->reference)) == 0, "Expected \"Hello\", (got %s)", mystruct->reference);
        return true;
    })();
...
TEST(customdtor)
{
    auto var obj = new(struct MyStruct, (void *)MyStruct_free);
    obj->num = 16;
