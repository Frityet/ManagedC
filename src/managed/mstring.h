...
{
	mstring *s = managed_from_pointer((void *)str, len, sizeof(mc_char_t), NULL);
...
#define mstr_a(str) managed_string_new(str, strlen(str))
/**
* Allocates and copies the str pointer
*/
static mstring *mc_nullable managed_string_new(const mc_char_t *mc_nonnull str, size_t len)
{
	mstring *s = managed_allocate(len, sizeof(mc_char_t), NULL, str);
	if (s == NULL)
		return NULL;

	_mcinternal_ptrinfo(s)->count--;	
...
static mstring *mc_nullable managed_string_duplicate(mstring *mc_nonnull str)
{
	...
	return managed_string_new(str, length);
}
...
static mstring *mc_nullable managed_string_concatenate(mstring *mc_nonnull s1, mstring *mc_nonnull s2, size_t s2len)
{
	...
	MC_MEMCPY((mc_char_t *)(s + s1len), s2, s2len);
	((mc_char_t *)s)[total] = '\0';
...
*/
static int managed_string_equals(mstring *mc_nonnull s1, const mc_char_t *mc_nonnull s2, long int s2len)
{
